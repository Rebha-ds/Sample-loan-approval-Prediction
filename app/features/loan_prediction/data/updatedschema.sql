...
ALTER TABLE `mydb`.`loan_predict_train` 
CHANGE COLUMN `Gender` `Gender` VARCHAR(10) NULL DEFAULT NULL ,
CHANGE COLUMN `Married` `Married` VARCHAR(10) NULL DEFAULT NULL ,
CHANGE COLUMN `Dependents` `Dependents` INT(11) NULL DEFAULT NULL ,
CHANGE COLUMN `Education` `Education` VARCHAR(45) NULL DEFAULT NULL ,
CHANGE COLUMN `Self_Employed` `Self_Employed` VARCHAR(20) NULL DEFAULT NULL ,
...
USE `mydb`$$
DROP TRIGGER IF EXISTS `mydb`.`loan_predict_train_BEFORE_INSERT` $$

USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`loan_predict_train_BEFORE_INSERT` BEFORE INSERT ON `loan_predict_train` FOR EACH ROW
BEGIN
